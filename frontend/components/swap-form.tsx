...
export const SwapForm: React.FC = () => {
  ...
  const onSubmit = async (data: SwapFormValues) => {
    setLoading(true)
    const { payAmount, payToken, receiveToken, slippage } = data;
    ...
    try {
      const quoteResponse = await axios.get("https://quote-api.jup.ag/v6/quote", {
        params: {
          inputMint: payTokenMetadata?.addresses['solana'],
          outputMint: receiveTokenMetadata?.addresses['solana'],
  ...
  return (
    <>
      <Modal isOpen={isOpen} title="" description="" onClose={() => setIsOpen(false)}>
        ...
        <div className="mt-2 flex h-60 flex-col gap-2 overflow-y-scroll px-2">
          {isEmpty(tokenList) ? (
            <>
              <Typography className="mt-8 w-full text-center" level="body3">
                No token found
              </Typography>
      ...
      <Form {...form}>
        ...
        >
          <div className="flex items-center justify-between rounded-2xl bg-white/60 p-2 shadow-card">
            <div onClick={() => form.reset()} className="cursor-pointer rounded-xl bg-white p-3 shadow-card hover:bg-gray-100">
              <UpdateIcon />
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0">
                  <Card className='w-fit p-2 flex flex-col bg-white'>
          ...
          <div className="rounded-2xl bg-white/60 p-6 shadow-card">
            <div className="flex flex-col gap-4">
              ...
              {connected ? 
                <Button 
                  loading={loading}
                  disabled={form.watch("payAmount") <= 0 || form.watch("receiveAmount") <= 0} type="submit"
                >
                  Swap
